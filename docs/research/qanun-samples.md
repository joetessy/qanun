...
## Chosen: sliced CC0 Bozkurt Turkish-kanun recording — wired in Phase 2

**Decision (2026-06-09):** Candidate 1 (Freesound #211133, barisbozkurt, CC0) was selected — unambiguously public-domain, real Turkish kanun with authentic trichord shimmer.

**What was wired:**
- 18 WAV files (per-note slices of the CC0 recording, pitch-detected + named) in `public/samples/qanun/`, covering F3–Ds6 (one every ~2 semitones). Attribution in `public/samples/qanun/NOTICE.md`.
- `src/lib/audio/qanunSamples.ts`: exports `QANUN_SAMPLE_URLS` (18-entry map, note name → filename) and `QANUN_SAMPLE_BASE_URL`.
- `createQanunEngine.ts`: `Tone.Sampler` → subtle `Tone.Chorus` (triple-course shimmer) → reverb → sumBus. Supports `soundSource: 'sample'|'synth'` toggle; falls back to PluckSynth pool while loading or when synth mode is active.
- Tone.Sampler pitch-shifts from the nearest sample, so all chromatic and microtonal (quarter-tone) notes work automatically.
